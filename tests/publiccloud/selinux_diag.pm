...
sub is_selinux_enabled {
    ...
    my $cmdline = $instance->ssh_script_output(
    ...
    my $selinux_dir_exists = $instance->ssh_script_output(
        cmd => 'stat /sys/kernel/security/selinux >/dev/null 2>&1; echo $?',
        proceed_on_failure => 1
    ) == 0;
...
sub upload_selinux_denials {
    ...
    my $denials = $instance->ssh_script_output(
        cmd => 'command -v ausearch >/dev/null 2>&1 && sudo ausearch -m avc,user_avc,selinux_err,user_selinux_err -ts today --raw || dmesg | grep -i "selinux.*denied"',
        proceed_on_failure => 1
    );
    if ($denials && $denials !~ /^\s*$/) {  # Check if we got non-empty output
        my $log_file = "/tmp/selinux_denials_" . time() . ".log";
        $instance->ssh_script_output(
            cmd => "echo '$denials' > $log_file",
            proceed_on_failure => 1
        );
