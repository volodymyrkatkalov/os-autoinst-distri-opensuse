...
my $EC2_CW_LOGS = [
    {
        ...
        filename => 'ec2__logs__messages.txt',
    },
    {
...
sub snapshot_ec2_serial_console
{
    ...
    my $timestamp = time();
    my $output_file = "ec2__serial_console__${timestamp}.txt";

    my $status = script_output(
    ...
    save_tmp_file($output_file, $status);
...
sub run {
    ...
    if (is_ec2()) {
        ...
        $self->install_ec2_cloudwatch_agent($instance);
    }
